...
Future<void> _openAddDialog(BuildContext context) async {
  ...
  final result = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      title: const Text('Dodaj režisera'),
      content: Form(
        key: formKey,
        child: SizedBox(
          width: 400,
          child: TextFormField(
            ...
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Unesite ime i prezime' : null,
          ),
        ),
      ),
      actions: [
  ...
  if (result == true) {
    await context.read<DirectorsCubit>().create(nameCtrl.text.trim());
  }
}
...
Future<void> _openEditDialog(BuildContext context, String id, String currentName) async {
  ...
  final result = await showDialog<bool>(
    context: context,
    builder: (ctx) => AlertDialog(
      title: const Text('Uredi režisera'),
      content: Form(
        key: formKey,
        child: SizedBox(
          width: 400,
  ...
  if (result == true) {
    await context.read<DirectorsCubit>().update(id, nameCtrl.text.trim());
  }
}
...
Future<void> _confirmDelete(BuildContext context, String id, String name) async {
  final confirmed = await showDialog<bool>(
    context: context,
    builder: (ctx) => AlertDialog(
      title: const Text('Brisanje režisera'),
      content: Text('Da li ste sigurni da želite obrisati režisera "$name"?'),
      actions: [
        TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Otkaži')),
        FilledButton(
  ...
  if (confirmed == true) {
    await context.read<DirectorsCubit>().delete(id);
  }
}

class _DirectorsContent extends StatefulWidget {
  const _DirectorsContent();

  @override
  State<_DirectorsContent> createState() => _DirectorsContentState();
}

class _DirectorsContentState extends State<_DirectorsContent> {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
                ...
                child: TextField(
                  ...
                  onChanged: (v) {
                    ...
                    _debounce = Timer(const Duration(milliseconds: 350), () {
                      if (!mounted) return;
                      context.read<DirectorsCubit>().load(page: 1, search: v);
                    });
                  },
                ),
              ),
          ...
          Expanded(
            child: BlocBuilder<DirectorsCubit, DirectorsState>(
                ...
                return Column(
                  children: [
                                              ...
                                              DataCell(Row(
                                                children: [
                                                  IconButton(
                                                    ...
                                                    onPressed: () => _openEditDialog(context, d.id, d.fullName),
                                                  ),
                                                  IconButton(
                                                    tooltip: 'Obriši',
                                                    icon: const Icon(Icons.delete_outline),
                                                    color: Theme.of(context).colorScheme.error,
                                                    onPressed: () => _confirmDelete(context, d.id, d.fullName),
                                                  ),
                                                ],
                                              )),
                                            ],
                    ...
                    _Pagination(
                      ...
                      onPageChanged: (p) => context.read<DirectorsCubit>().load(page: p),
                    ),
                  ],
                );
              },
