...
Future<void> _openAddDialog(BuildContext context) async {
  ...
  if (result == true) {
    await context.read<GenresCubit>().create(nameCtrl.text.trim());
  }
}
...
Future<void> _openEditDialog(BuildContext context, String id, String currentName) async {
  ...
  if (result == true) {
    await context.read<GenresCubit>().update(id, nameCtrl.text.trim());
  }
}
...
Future<void> _confirmDelete(BuildContext context, String id, String name) async {
  final confirmed = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: Text('Da li ste sigurni da želite obrisati žanr "$name"?'),
      actions: [
        TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Otkaži')),
        FilledButton(
  ...
  if (confirmed == true) {
    await context.read<GenresCubit>().delete(id);
  }
}

class _GenresContent extends StatefulWidget {
  const _GenresContent();

  @override
  State<_GenresContent> createState() => _GenresContentState();
}

class _GenresContentState extends State<_GenresContent> {
                ...
                child: TextField(
                  ...
                  onChanged: (v) {
                    ...
                    _debounce = Timer(const Duration(milliseconds: 350), () {
                      if (!mounted) return;
                      context.read<GenresCubit>().load(page: 1, search: v);
                    });
                  },
                ),
              ),
                ...
                return Column(
                  children: [
                                              ...
                                              DataCell(Row(
                                                children: [
                                                  IconButton(
                                                    ...
                                                    onPressed: () => _openEditDialog(context, g.id, g.name),
                                                  ),
                                                  IconButton(
                                                    tooltip: 'Obriši',
                                                    icon: const Icon(Icons.delete_outline),
                                                    color: Theme.of(context).colorScheme.error,
                                                    onPressed: () => _confirmDelete(context, g.id, g.name),
                                                  ),
                                                ],
                                              )),
                                            ],
                    ...
                    _Pagination(
                      ...
                      onPageChanged: (p) => context.read<GenresCubit>().load(page: p),
                    ),
                  ],
                );
              },
