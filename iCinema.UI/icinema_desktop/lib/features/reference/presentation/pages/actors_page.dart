...
Future<void> _openAddDialog(BuildContext context) async {
  ...
  if (result == true) {
    await context.read<ActorsCubit>().create(nameCtrl.text.trim());
  }
}
...
Future<void> _openEditDialog(BuildContext context, String id, String currentName) async {
  ...
  if (result == true) {
    await context.read<ActorsCubit>().update(id, nameCtrl.text.trim());
  }
}
...
Future<void> _confirmDelete(BuildContext context, String id, String name) async {
  final confirmed = await showDialog<bool>(
    context: context,
    builder: (ctx) => AlertDialog(
      title: const Text('Brisanje glumca'),
      content: Text('Da li ste sigurni da želite obrisati glumca "$name"?'),
      actions: [
        TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Otkaži')),
        FilledButton(
  ...
  if (confirmed == true) {
    await context.read<ActorsCubit>().delete(id);
  }
}

class _ActorsContent extends StatefulWidget {
  const _ActorsContent();

  @override
  State<_ActorsContent> createState() => _ActorsContentState();
}

class _ActorsContentState extends State<_ActorsContent> {
                ...
                child: TextField(
                  ...
                  onChanged: (v) {
                    ...
                    _debounce = Timer(const Duration(milliseconds: 350), () {
                      if (!mounted) return;
                      context.read<ActorsCubit>().load(page: 1, search: v);
                    });
                  },
                ),
              ),
                ...
                return Column(
                  children: [
                    ...
                    _Pagination(
                      ...
                      onPageChanged: (p) => context.read<ActorsCubit>().load(page: p),
                    ),
                  ],
                );
              },
